...
case class AggregateStats(cpus: Int = 0, memoryMiBs: Int = 0, storageMiBs: Int = 0,
  ...
  def aggregate(ls: LocalStats) = AggregateStats(
    cpus = this.cpus + ls.processing.numCPUs,
    memoryMiBs = this.memoryMiBs + ls.memory.MiBs,
    storageMiBs = this.storageMiBs + ls.storage.MiBs,
    cpusInUse = this.cpusInUse + ls.processing.numCPUsInUse,
    memoryMiBsInUse = this.memoryMiBsInUse + ls.memory.MiBsInUse,
    storageMiBsInUse = this.storageMiBsInUse + ls.storage.MiBsInUse,
  )
}
...
class AdhocCloud extends AggregateProgram
  ...
  def metricInjecter(k: Int): Injecter[Double] = () =>
    if(mid != 0 || k < metricInjectedAt){ Fun(0, ap => ap.nbrRange()) } else { Fun(1, ap => ap.nbrRange()+env.get[Double]("metricDelta")) }

  lazy val metricInjectedAt: Int = env.get[Int]("metricInjectedAt")
  ...
  override def main(): Any = {
    ...
    val dynamicRandom = rep((20+(nextRandom*changeEveryMaxRounds).toInt, nextRandom)) { case (step,rand) =>
      ...
      if(k % step == 0) (step,nextRandom + (if((k>200 && k<400) || (k>500 && k <600)) 0.4 else 0)) else (step,rand)}._2
    ...
    val leaders = branch(env.get[Boolean](IsLeader) & nextRandom<env.get[Double]("failureProb")){
      env.put("failed", env.get[Int]("failed")+1)
      false
    }{
      S(env.get[Double]("grain"), metric)
    }
    val potential = distTo(leaders) // metric

    val resourcesPerArea = C[Double,Stats](potential, _++_, Map(mid -> localResources), Map())
    val aggregatedResourcesPerArea = resourcesPerArea.values.foldLeft(AggregateStats())(_.aggregate(_))
    ...
    val myLeader = broadcast(potential, metric, mid)
    ...
    val adjacentAreasStats: Set[AreaStats] = broadcast(potential, metric, C[Double,Set[AreaStats]](potential, _.union(_), nbrAreas, Set()))
  ...
  def G[V](g: Double, metric: ()=>Double, field: V, acc: V => V): V = {
    rep(field) { case (value) =>
  ...
  def broadcast[V](potential: Double, metric: () => Double, field: V): V =
    G(potential, metric, field, identity(_:V))
